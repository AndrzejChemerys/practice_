function isLucky(n: number): boolean {
    ...
    // const firstHalf = luckyN.substring(0, half).split('').map((char) => parseInt(char)).reduce((num1, num2) => num1+num2);

    // console.log(firstHalf)

    // const secondHalf = luckyN.substring(half, luckyN.length).split('').map((char) => parseInt(char)).reduce((num1, num2) => num1 + num2);
